...
export function ChatInterface({
      ...
      setMessages([
        {
          id: "error",
          content:
            "Sorry, I couldn't load the chat history. Let's start fresh!",
          role: "assistant",
          timestamp: new Date(),
        },
      ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorText =
        "I apologize, but I'm having trouble processing your request right now. Please try again later, or contact support if the problem persists.";
          ...
          <div className="relative">
            ...
            {isListening && (
              <div className="absolute -inset-2 pointer-events-none">
                ...
                <div
                  className="absolute inset-1 rounded-full border-2 border-red-300 animate-ping opacity-30"
                  style={{ animationDelay: "0.2s" }}
                ></div>
                <div
                  className="absolute inset-2 rounded-full border-2 border-red-200 animate-ping opacity-20"
      ...
      {!hasMessages ? (
        ...
        <>
          ...
          <div className="bg-gradient-to-r from-white via-gray-50 to-white border-t border-gray-200 flex-shrink-0 backdrop-blur-sm">
            <div className="max-w-5xl mx-auto p-6">{renderInputForm()}</div>
          </div>
        </>
      )}
    </div>
